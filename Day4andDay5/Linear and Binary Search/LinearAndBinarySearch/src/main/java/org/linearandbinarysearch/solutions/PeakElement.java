package org.linearandbinarysearch.solutions;
...
public class PeakElement {
    ...
    public int search(int[] arr, int left, int right) {
        ...
        if(left == right) {
            if(left == arr.length-1 || left == 0) {
                // means the peak element is not found and terminal elements cannot be peak elements
                return -1;
            }
        }
